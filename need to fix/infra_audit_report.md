...
The `backend.tf` file itself calls this out: *"Local state is NOT acceptable for this directory. Until the backend below is configured, treat `terraform apply` as blocked."* Yet the backend remains commented.
...
**What's wrong:**
The lock file comment on line 4 explicitly states: *"Platform coverage: this lock file was regenerated for linux_amd64 (CI) only."* The hashes array contains only 9 entries — consistent with a single-platform generation. `darwin_arm64` (macOS M-series) and `windows_amd64` hashes are absent.
