...
- **Missing Input Validation:** Some API endpoints bypass Zod schema wrappers (e.g., `withValidation`) and manually cast `request.json()`.
  - *File:* `src/app/api/patient/documents/route.ts` (Lines 187-200)
  - *Problem:* `const input = body as Record<string, unknown>;` relies on manual `typeof` checks instead of a robust Zod schema. This risks missing edge cases or allowing prototype pollution.
  - *Fix:* Define a Zod schema and wrap the handler in `withValidation()`.
...
---
*I will automatically update this report as soon as the `typecheck` and `eslint` background tasks finish.*
