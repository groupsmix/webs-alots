...
--
-- Skips entirely on environments where the seed clinic was never
-- inserted (e.g. fresh `supabase start` in CI, which only runs
-- migrations + seed.sql and does not load supabase/seeds/*).
-- ============================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Skipping 00040: seed clinic c1000000-...0001 does not exist (expected on a fresh DB without supabase/seeds applied).';
    RETURN;
  END IF;
