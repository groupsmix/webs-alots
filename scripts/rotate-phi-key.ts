 ...
 *
 * Usage:
 *   npx tsx scripts/rotate-phi-key.ts [--dry-run] [--force] [--secret-type <type>]
 *
 * Options:
 *   --dry-run           Simulate rotation without making changes
 *   --force             Skip age checks and rotate immediately
 *   --secret-type       Type of secret to rotate: phi-encryption (default), r2-signed-url, all
 *
 * Cron Trigger:
 *   This script can be invoked from a cron job for automated rotation.
 *   Add to wrangler.toml:
 *     [[triggers]]
 *     crons = ["0 0 1 */3 *"]  # First day of every quarter at midnight UTC
 *
 *   The cron handler should invoke this script with appropriate environment variables.
 *   See docs/SOP-SECRET-ROTATION.md for full automation setup.
 *
 * Rotation Cadence:
 *   - Automated: Every 90 days via cron trigger
 *   - Manual: On-demand with --force flag
 *   - Health check fails if key age > 97 days (7-day grace period)
 *
 * The script:
...
async function main() {
  ...
  const secretTypeArg = process.argv.find((arg) => arg.startsWith("--secret-type="));
  const secretType = secretTypeArg ? secretTypeArg.split("=")[1] : "phi-encryption";

  // Validate secret type
  const validSecretTypes = ["phi-encryption", "r2-signed-url", "all"];
  if (!validSecretTypes.includes(secretType)) {
    console.error(`ERROR: Invalid secret type "${secretType}". Valid types: ${validSecretTypes.join(", ")}`);
    process.exit(1);
  }
  ...
  console.log(`Force: ${force ? "YES (skip age checks)" : "NO"}`);
  console.log("");

  // Check rotation cadence enforcement (skip if --force)
  if (!force && !dryRun) {
    console.log("Checking rotation cadence...");
    // In production, this would check the last rotation timestamp from KV or audit logs
    // For now, we'll just log a warning
    console.log("WARNING: Rotation cadence enforcement not yet implemented.");
    console.log("In production, this would verify the key is >90 days old before rotating.");
  ...
  if (!dryRun) {
    ...
    console.log("  4. Update rotation timestamp in KV for cadence enforcement");
    console.log("\nAutomated Rotation:");
    console.log("  - This script can be invoked from a cron job for automated rotation");
    console.log("  - Add cron trigger to wrangler.toml: [[triggers]].crons = [\"0 0 1 */3 *\"]");
    console.log("  - See docs/SOP-SECRET-ROTATION.md for full automation setup");
  }
}
