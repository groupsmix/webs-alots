...
| P3 — Identity modeling (auth roles vs specialist slugs)  | ✅ Resolved | Security gap was already fixed (specialist routes gated to staff roles). The modeling question is now resolved: `src/lib/config/capabilities.ts` is the single canonical identity source. It defines the 5 `CoreRole`s, a `Capability` union, a `CAPABILITIES` role→capability map, and one canonical slug↔capability mapping that resolves the `speech-therapist`/`speech_therapist` and `secretary`/`receptionist` drift. Specialists/pharmacist remain **capabilities layered on the 5 core roles (NOT new DB roles)**, matching the live `SPECIALIST_STAFF_ROLES` gating in `src/middleware.ts`. `SPECIALIST_PROTECTED_PREFIXES` (routes.ts) and `PROTECTED_ROUTE_PREFIXES` (next.config.ts) are now DERIVED from `capabilities.ts`; the AI persona alias references the same `ROLE_TO_PERSONA` mapping. Drift is prevented by `src/lib/__tests__/capabilities.test.ts`. No new DB roles, no migrations. See §17 of `project_architecture_analysis(2).md`. |
...
### 1. ✅ Auth Roles vs Specialist Slugs — Unified (P3, RESOLVED)

> **Resolved.** The two identity systems below are now unified behind a single canonical layer, `src/lib/config/capabilities.ts`. Specialist capability is modeled as **capabilities layered on the 5 core roles** (not new DB roles), the derived route-prefix lists are generated from that one map, and the AI persona alias references the same mapping. The historical split is retained here for context.
...
- **No drift by construction:** `SPECIALIST_PROTECTED_PREFIXES` (SEALED `routes.ts`, import-only) and `PROTECTED_ROUTE_PREFIXES` (`next.config.ts`) are derived from `capabilities.ts`; `src/lib/__tests__/capabilities.test.ts` asserts full derivation and fail-closed behaviour for unknown roles/slugs.
...
```mermaid
graph TD
    P3["✅ P3: Identity unified<br/>capabilities.ts — one canonical source"]
...
| -------- | --------------------------------- | ----------------------- | ------ | ------ |
| ✅ P3    | Role/slugs/persona identity drift | RESOLVED — unified in `capabilities.ts` | — | Done |
