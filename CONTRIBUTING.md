...
2. **REQUIRED**: Use `withAuth()`, `withAuthAnyRole()`, or `withAuthValidation()` wrappers from `@/lib/with-auth` and `@/lib/api-validate`
   - **Never call `createClient()` directly in route handlers** — this bypasses authentication and RBAC checks (A7-05)
   - The ESLint rule `custom/no-direct-supabase-in-routes` enforces this requirement
   - If you need to create a public endpoint (e.g., health check, webhook), see the exception process below
...
### ESLint Rule Exception Process (A7-05)

The `custom/no-direct-supabase-in-routes` rule prevents authentication bypass by requiring all route handlers to use authentication wrappers. If you need to create a **legitimate public endpoint** (e.g., health check, webhook with signature verification), follow this process:

1. **Document the justification** — Add a comment explaining why the endpoint must be public
2. **Request security review** — Tag `@security-team` in your PR description
3. **Add `eslint-disable` comment** — Only after security approval:

```typescript
// eslint-disable-next-line custom/no-direct-supabase-in-routes -- Public health check endpoint (approved by security team, PR #123)
const supabase = await createClient();
```

4. **Implement alternative security controls** — Public endpoints must have:
   - Rate limiting (via middleware or route-level checks)
   - Input validation (Zod schemas)
   - Signature verification (for webhooks)
   - No access to tenant-scoped data without explicit authorization
...
- `/api/webhooks/whatsapp` — Webhook with Meta signature verification

**Examples that should NOT be exceptions:**
- Any endpoint that queries patient data
- Any endpoint that modifies clinic data
- Any endpoint that requires user authentication
