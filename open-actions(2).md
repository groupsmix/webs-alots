...
| ~~A6-13~~ | ~~MEDIUM~~ | ~~Booking token HMAC lacks clinicId — cross-tenant replay possible~~ | ~~`src/app/api/booking/route.ts`~~ | ~~FIXED: Token format is now `clinicId:phone:expiry:signature` with `clinicId` in the signed HMAC payload. Old 3-part tokens are rejected. The route verifies `tokenResult.clinicId === clinicId` before proceeding.~~ | — | — | OWASP ASVS §6.2.3, tenancy isolation. |
...
| ~~A14-02~~ | ~~LOW~~ | ~~`bookingVerifySchema.phone`~~ | ~~`src/lib/validations.ts`~~ | ~~FIXED: Added `.regex(/\^\+?[0-9()\s-]+$/, "Invalid phone format")`.~~ | — |
| ~~A14-03~~ | ~~LOW~~ | ~~`labReportSchema.results[].testName`~~ | ~~`src/lib/validations.ts`~~ | ~~FIXED: `safeName.pipe(z.string().min(1).max(200))`.~~ | — |
...
| ~~A14-06~~ | ~~LOW~~ | ~~Locale cookie~~ | ~~`src/app/api/lab/report-html/route.ts`~~ | ~~FIXED: `decodeURIComponent` wrapped in try/catch with fallback to `DEFAULT_LOCALE`.~~ | — |
...
- ~~**F-A95-06** (MED)~~ ~~The duplicate-numbered migration `00072_*`~~ ~~(see F-A99-02) was merged across two PRs without conflict — *because* the filenames differ (only the prefix collides).~~ ~~FIXED 2026-05-08: renamed duplicates to `00079_*` and `00080_*`; CI check in `.github/workflows/migration-check.yml` already rejects duplicate prefixes.~~
...
- ~~**F-A96-01** (HIGH)~~ ~~**TOCTOU is still present.** `src/app/api/booking/route.ts` uses *post-insert compensation*~~ ~~(insert, then count, then delete).~~ ~~FIXED: The route handler now calls `booking_atomic_insert` RPC which wraps slot-count check + insert inside a single transaction with `pg_advisory_xact_lock`. No compensation block remains.~~
- ~~**F-A96-02** (HIGH)~~ ~~**Notifications cross-tenant query.** `src/app/api/notifications/route.ts`~~ ~~queries without `.eq("clinic_id", clinicId)`.~~ ~~FIXED: The route handler now applies `.eq("clinic_id", profile.clinic_id)` on both POST and GET queries as defense-in-depth alongside RLS.~~
...
- ~~**F-A96-04** (HIGH)~~ ~~**Duplicate migration prefix `00072`** (`00072_booking_slot_advisory_lock.sql` and `00072_notifications_clinic_id.sql`).~~ ~~FIXED 2026-05-08: renamed `00072_notifications_clinic_id.sql` → `00079_notifications_clinic_id.sql` and `00073_appointments_status_slot_start_index.sql` → `00080_appointments_status_slot_start_index.sql`. All migration prefixes are now unique. CI check in `.github/workflows/migration-check.yml` enforces uniqueness.~~
...
## ~~A97 — Hypothetical CVE Advisory (HN frontpage tomorrow)~~ — **FIXED 2026-05-08**
...
> **Status:** FIXED — The route handler now calls `supabase.rpc("booking_atomic_insert", {...})` which wraps the slot-count check + insert inside a single transaction with `pg_advisory_xact_lock`, eliminating the TOCTOU race.
> **Affected versions:** main (HEAD `9edc2adf` and prior). **Fixed in current main.**
> **Root cause:** ~~`src/app/api/booking/route.ts` enforces `maxPerSlot` via *post-insert compensation* rather than within a single transaction or advisory-lock.~~ ~~RESOLVED~~ — Atomic RPC is now wired into the booking handler.
>
> **Proof of concept:**
> ```bash
...
|---|---|---|---|---|---|
| `POST /api/chat` (basic/smart/advanced) | Optional (basic public, smart/advanced require auth) | 15/min/IP | inline `sanitizeUserInput` | **checked** (`isAIEnabled` via `resolveAIConfig` in advanced; direct check in smart) | gpt-4o-mini / `@cf/meta/llama-3.1-8b-instruct` |
| `POST /api/ai/auto-suggest` | doctor | 100/day/doctor | `sanitizeUntrustedText` + UNTRUSTED block | **checked** (`isAIEnabled`) | gpt-4o-mini |
| `POST /api/ai/manager` | clinic_admin | 30/day/admin | `sanitizeUntrustedText` (partial) | **checked** (`resolveAIConfig`) | gpt-4o-mini |
| `POST /api/ai/whatsapp-receptionist` | webhook (HMAC-SHA256) | 200/60s/clinic (`webhookLimiter`) | `sanitizeUntrustedText` + UNTRUSTED block | **checked** (`resolveAIConfig`) | gpt-4o-mini |
| `POST /api/v1/ai/prescription` | doctor | 50/day/doctor | `sanitizeUntrustedText` + UNTRUSTED block | **checked** (`resolveAIConfig`) | gpt-4o-mini |
| `POST /api/v1/ai/patient-summary` | doctor | 30/day/doctor | `sanitizeUntrustedText` + UNTRUSTED block | **checked** (`resolveAIConfig`) | gpt-4o-mini |
| `POST /api/v1/ai/drug-check` | doctor | 100/day/doctor | `sanitizeUntrustedText` + UNTRUSTED block | **checked** (`resolveAIConfig`) | gpt-4o-mini |
...
| Kill switch | **PASS** | `isAIEnabled()` exists in `src/lib/features.ts:153` and is enforced via `resolveAIConfig()` in all OpenAI routes (`auto-suggest`, `manager`, `whatsapp-receptionist`, `prescription`, `patient-summary`, `drug-check`). The `chat` route smart tier now also checks `isAIEnabled()` directly before calling Cloudflare Workers AI (fixed 2026-05-08). Operator can disable all AI traffic in a single flag flip. |
| Abuse classifier | **FAIL** | None on inbound or outbound. |


**Overall A107: PARTIAL.** A107-1 (kill-switch) **FIXED 2026-05-08** — all 6 AI routes now enforce `isAIEnabled()`. Remaining gaps: rollback, A/B shadow, abuse classifier (not addressed in this pass).
...
| 20 | Kill-switch evasion | Set FEATURE_FLAGS_KV `ai.enabled = "false"` and call `/api/v1/ai/prescription` | **blocked** | All AI routes now consult `resolveAIConfig()` which wraps `isAIEnabled()`; `chat` smart tier checks directly (fixed 2026-05-08). |

## Top 10 Actionable Findings (priority-ordered)

| # | Finding | Severity | Effort | Where |
|---|---|---|---|---|
| ~~1~~ | ~~Kill switch enforced in only 1 of 6 AI routes.~~ | ~~HIGH~~ | ~~XS~~ | ~~FIXED 2026-05-08: All OpenAI routes use `resolveAIConfig()` (wraps `isAIEnabled()`); `chat` smart tier checks directly.~~ |
| 2 | **Stored prompt injection via `chatbot_faqs.answer` / `services.name` / `users.name`.** A clinic_admin (or anyone exploiting a future SQL bug) plants instructions read by every patient. | **HIGH** | S | **FIXED 2026-05-08**: `chatbot-data.ts` now sanitizes `services.name`, `doctors.name`, `clinic.name`, and all clinic contact fields via `sanitizeUntrustedText()` before they enter the system prompt. `whatsapp-receptionist/route.ts` sanitizes `ctx.name`, `ctx.phone`, `ctx.address`, `ctx.services`, `ctx.doctors`. `prescription/route.ts` already sanitized `patient.name`. |
...
| ~~5~~ | ~~`OPENAI_BASE_URL` is operator-overridable with no allowlist.~~ | ~~MEDIUM~~ | ~~XS~~ | ~~FIXED: `src/lib/ai/config.ts` enforces `ALLOWED_AI_BASE_URLS` (api.openai.com + *.openai.azure.com). Unknown URLs return 500.~~ |
| ~~6~~ | ~~UNTRUSTED delimiter not stripped from user input.~~ | ~~MEDIUM~~ | ~~XS~~ | ~~FIXED: `sanitizeUntrustedText()` now strips `--- UNTRUSTED ---`, `[UNTRUSTED]`, and `<<UNTRUSTED_*>>` markers.~~ |
| ~~7~~ | ~~No model version pinning.~~ | ~~MEDIUM~~ | ~~XS~~ | ~~FIXED: `src/lib/ai/config.ts` pins `DEFAULT_MODEL = "gpt-4o-mini-2024-07-18"`. `resolveAIConfig()` falls back to this pinned version.~~ |
| ~~8~~ | ~~`logAuditEvent` not called for AI invocations.~~ | ~~MEDIUM~~ | ~~S~~ | ~~FIXED: `prescription/route.ts` and `drug-check/route.ts` call `logAuditEvent()` after AI success with tokens_used, medication_count, ai_enhanced. No prompt/completion bodies logged (PHI safe).~~ |
...
| ~~F-A95-06 / F-A96-04~~ | ~~Two migration files share `00072_*` prefix (numbering collision)~~ | ~~MED~~ | ~~FIXED 2026-05-08: renamed duplicates; CI rule already present in `.github/workflows/migration-check.yml`.~~ | ~~DB Eng~~ | ~~—~~ |
| ~~F-A96-01 / F-A99-01~~ | ~~`booking_atomic_insert` RPC defined but compensation-block still in route handler~~ | ~~MED~~ | ~~FIXED: route handler now calls `booking_atomic_insert` RPC directly.~~ | ~~API Eng~~ | ~~—~~ |
...
| ~~F-A160~~ | ~~No OFAC/EU/UK/UN sanctions screening~~ | ~~HIGH~~ | ~~FIXED: `src/lib/middleware/sanctioned-countries.ts` blocks CU, IR, KP, SY, RU at middleware via Cloudflare `request.cf.country`. Returns 451.~~ | ~~Sec Eng~~ | ~~—~~ |
| ~~F-A164-a~~ | ~~`invoice_number` is free-form `TEXT`, no `SEQUENCE`~~ | ~~HIGH~~ | ~~FIXED: Migration `00081_invoice_sequence.sql` adds `invoice_sequences` counter table with advisory-lock RPC `next_invoice_number()`, unique constraint, and `FAC-YYYY-NNNNN` format check.~~ | ~~DB Eng~~ | ~~—~~ |
...
| ~~F-A167~~ | ~~No DB-level audit triggers on money tables~~ | ~~HIGH~~ | ~~FIXED: Migration `00082_money_audit_triggers.sql` adds AFTER INSERT/UPDATE/DELETE triggers on `payments`, `invoices`, `billing_events` that write to `activity_logs` with actor resolution from JWT or DB role.~~ | ~~DB Eng~~ | ~~—~~ |
...
| ~~F-A169~~ | ~~Refund not race-safe~~ | ~~HIGH~~ | ~~FIXED: Migration `00083_refund_safety.sql` adds `refunded_amount` CHECK constraint, `refund_idempotency_key` unique index, and `safe_refund()` RPC with `SELECT ... FOR UPDATE`. Route handler uses `process_payment_refund` RPC.~~ | ~~Billing Eng~~ | ~~—~~ |
| ~~F-A188~~ | ~~`audit_logs` not WORM/immutable~~ | ~~HIGH~~ | ~~FIXED: Migration `00086_worm_audit_r2_sink.sql` adds `audit_archive_queue`, `trg_activity_logs_archive` (queues inserts for R2), `trg_activity_logs_immutable_update/delete` (blocks UPDATE/DELETE with exception), and REVOKE on authenticated/anon.~~ | ~~Sec Eng~~ | ~~—~~ |
| ~~F-A199~~ | ~~AI medical-advice has no user-visible disclaimer string~~ | ~~HIGH~~ | ~~FIXED: `prescription/route.ts` and `drug-check/route.ts` spread `AI_RESPONSE_DISCLAIMER` (from `src/lib/ai/config.ts`) into all success responses. French disclaimer: "Cette information est générée par une IA..."~~ | ~~AI Eng + Clinical Lead~~ | ~~—~~ |
| ~~F-A200~~ | ~~No parental-consent path for minor patients~~ | ~~HIGH~~ | ~~FIXED: Migration `00084_guardian_patient.sql` adds `guardian_user_id` FK, `is_minor` boolean, `guardian_consent_obtained` boolean, auto-compute trigger, indexes, and `minors_missing_guardian_consent` view.~~ | ~~API Eng + DPO~~ | ~~—~~ |
| ~~F-A198~~ | ~~No country-block for sanctioned jurisdictions in middleware~~ | ~~HIGH~~ | ~~FIXED: `src/middleware.ts:154` calls `checkSanctionedCountry(request)` which returns 451 for OFAC-sanctioned countries (CU, IR, KP, SY, RU).~~ | ~~Sec Eng~~ | ~~—~~ |
...
| F-AI-01 | Kill switch enforced in only 1 of 6 AI routes | **FIXED** | All 6 AI routes now enforce `isAIEnabled()` via `resolveAIConfig()` or direct check (2026-05-08). | AI Eng | — |
| F-AI-02 | Stored prompt injection via `chatbot_faqs.answer` / `services.name` / `users.name` | **HIGH** | **FIXED 2026-05-08**: All DB strings entering system prompts are now sanitized via `sanitizeUntrustedText()` — `chatbot-data.ts` (services, doctors, clinic fields, FAQs), `whatsapp-receptionist/route.ts` (ctx fields), `prescription/route.ts` (patient.name). | AI Eng | — |
...
| ~~F-AI-05~~ | ~~`OPENAI_BASE_URL` operator-overridable, no allowlist~~ | ~~MED~~ | ~~FIXED: `src/lib/ai/config.ts` enforces `ALLOWED_AI_BASE_URLS` with exact match + Azure OpenAI pattern. Unknown URLs return 500.~~ | ~~Sec Eng~~ | ~~—~~ |
| ~~F-AI-06~~ | ~~UNTRUSTED delimiter not stripped from user input~~ | ~~MED~~ | ~~FIXED: `sanitizeUntrustedText()` strips `--- UNTRUSTED ---`, `[UNTRUSTED]`, `<<UNTRUSTED_*>>` markers.~~ | ~~AI Eng~~ | ~~—~~ |
| ~~F-AI-07~~ | ~~No model version pinning (default `gpt-4o-mini` alias)~~ | ~~MED~~ | ~~FIXED: `src/lib/ai/config.ts` pins `DEFAULT_MODEL = "gpt-4o-mini-2024-07-18"`. `resolveAIConfig()` returns this pinned model.~~ | ~~AI Eng~~ | ~~—~~ |
| ~~F-AI-08~~ | ~~`logAuditEvent` not called for AI invocations~~ | ~~MED~~ | ~~FIXED: `prescription/route.ts` and `drug-check/route.ts` call `logAuditEvent()` after AI success. No prompt/completion bodies logged.~~ | ~~AI Eng~~ | ~~—~~ |
