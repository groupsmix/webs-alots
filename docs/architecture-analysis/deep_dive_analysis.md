...
> **Analysis note:** This file tracks structural conflicts and resolution status. It is not the living architecture overview.
...
## Part 2: Resolved Findings (2026-07-06 — Second Pass)

All items below were open after the first pass. They have now been fully resolved.

### 1. ✅ Auth Roles vs Specialist Slugs — Fully Resolved (P3)

> **Fully resolved (2026-07-06).** The identity _vocabulary_ and the _route-prefix drift_ were unified behind `src/lib/config/capabilities.ts` (first pass). The remaining enforcement follow-up — wiring `src/middleware.ts` to consume the `CAPABILITIES` map — has now been completed. `SPECIALIST_STAFF_ROLES.some(...)` is replaced with `capabilityForSlug(slug)` + `roleHasCapability(profile.role, capability)`. Both the vocabulary and the runtime gating now derive from the single canonical source.
...
- **✅ Enforcement wired (2026-07-06):** `src/middleware.ts` now calls `capabilityForSlug(slug)` + `roleHasCapability(profile.role, capability)` to gate specialist routes. The slug is extracted from the first path segment, resolved to a typed `Capability`, and checked against the profile role. Unknown slugs fail-closed. `SPECIALIST_STAFF_ROLES` is no longer referenced.

### 2. ✅ Migration 00187 vs Living Product Boundary (P9) — Resolved

The contradiction between migration 00187 (which declared "Oltigo is not an EMR" and dropped 12 clinical tables) and the surviving API routes that referenced those tables has been resolved by **deleting the 9 dead API route directories**:

- `src/app/api/prescriptions/`, `src/app/api/vitals/`, `src/app/api/radiology/`, `src/app/api/insurance-claims/`, `src/app/api/admissions/`, `src/app/api/pets/`, `src/app/api/menus/`, `src/app/api/restaurant-orders/`, `src/app/api/restaurant-tables/`

`validations/index.ts` gated re-exports removed. `src/lib/config/verticals.ts` `enabledApiGroups` cleared to `[]` for all vertical scopes. This permanently commits to Architecture A (operations-first).

### 3. ✅ `withAuth` / `withAuthAnyRole` Duplication (P8) — Resolved

`src/lib/with-auth.ts` had two long wrappers (`withAuth` and `withAuthAnyRole`) that duplicated ~130 lines of identical auth-resolution logic.
...
## Summary: All Priorities Resolved

```mermaid
graph TD
    P3["✅ P3: Identity vocabulary + enforcement unified\ncapabilities.ts + middleware.ts wired"]
    P8["✅ P8: withAuth deduplicated\nresolveAuthProfile() shared helper"]
    P9["✅ P9: Product boundary resolved\nDead API routes + schemas deleted"]
    P11["✅ P11: env.ts split into\ndomain-grouped leaf modules"]
...
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------------ |
| ✅ P3    | Role/slugs/persona identity drift | Fully resolved — vocabulary unified in `capabilities.ts`, runtime enforcement wired in `middleware.ts` |
| ✅ P8    | `withAuth` duplication            | Resolved — shared `resolveAuthProfile()` helper, public API unchanged                                  |
| ✅ P9    | Product-boundary sprawl           | Resolved — 9 dead API route directories deleted, validation exports removed                            |
| ✅ P11   | `env.ts` monolith                 | Resolved — barrel + domain-grouped leaf modules, public API unchanged                                  |
