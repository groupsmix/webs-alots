...
import { fetchCustomDomains, type CustomDomain } from "@/lib/data/custom-domains";
...
export default function CustomDomainsClient({
  ...
  async function refresh() {
    try {
      const data = await fetchCustomDomains(clinicId);
      setDomains(data);
      setError(null);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to refresh domains");
    }
  }
                      ...
                      <div>
                        ...
                        {domain.verification_txt && domain.status === "pending" && (
                          <p className="mt-1 text-xs text-muted-foreground">
                            TXT Record:{" "}
                            <code className="rounded bg-muted px-1">{domain.verification_txt}</code>
                          </p>
                        )}
                      </div>
                    </div>
