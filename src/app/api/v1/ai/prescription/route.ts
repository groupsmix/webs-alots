...
export const POST = withAuthValidation(
  ...
  async (data, _request: NextRequest, auth) => {
    ...
    const estimatedTokens = estimateTokens(data.diagnosis + (data.symptoms ?? "")) + 1000; // Add buffer for system prompt
    ...
    // F-AI-02: sanitize patient.name before interpolating into the system prompt
    const userMessage = buildPatientContext(
      { diagnosis: data.diagnosis, symptoms: data.symptoms, patientContext: mergedContext },
      sanitizeUntrustedText(patient.name ?? "Patient"),
    );
    ...
    try {
      ...
      // F-AI-08: Audit log the AI invocation (tokens, model — never prompt/completion bodies)
      void logAuditEvent({
        supabase,
        action: "ai.prescription_generated",
        type: "patient",
        clinicId,
        actor: doctorId,
        description: "AI prescription draft generated",
        metadata: {
      ...
      // F-AI-04 / A199: Include mandatory clinical disclaimer in all prescription AI responses.
      return apiSuccess({
        prescription,
