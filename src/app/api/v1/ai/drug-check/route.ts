...
async function checkWithAi(
  ...
  const estimatedTokens = estimateTokens(medications.join(", ")) + 500; // Add buffer for system prompt

  const { allowed } = await checkAiTokenBudget(
    supabase,
    clinicId,
    role,
    estimatedTokens,
  ...
  if (!allowed) {
    return null; // Silently fail to local check only
  }
...
export const POST = withAuthValidation(
  ...
  async (data, _request: NextRequest, auth) => {
    ...
    if (data.useAiFallback !== false && data.medications.length > 1) {
      const aiResult = await checkWithAi(data.medications, patientAllergies, clinicId, profile.role, supabase);
    ...
    // F-AI-08: Log AI invocation for audit trail (tokens, model — no PHI in body)
    void logAuditEvent({
      supabase,
      action: "ai.drug_check",
      type: "patient",
      clinicId,
      actor: doctorId,
      description: "AI drug interaction check performed",
      metadata: {
        ai_enhanced: aiEnhanced,
        medication_count: data.medications.length,
    ...
    // F-AI-04 / A199: Include mandatory medical disclaimer in every AI drug-check response.
