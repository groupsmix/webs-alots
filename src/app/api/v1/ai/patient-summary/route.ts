...
import { watermarkAiContent } from "@/lib/ai-watermark";
...
export const POST = withAuthValidation(
  ...
  async (data, _request: NextRequest, auth) => {
    ...
    // A1-01: Check AI token budget before making expensive AI call
    // Estimate tokens based on patient context size
    const estimatedTokens = 1500; // Patient summaries typically use ~1500 tokens
    ...
    // F-AI-02: sanitize patient.name before interpolating into the prompt
    const userMessage = buildUserMessage({
      patient: {
        name: sanitizeUntrustedText(patientData.name ?? "Patient"),
    ...
    try {
      ...
      await incrementAiTokenUsage(supabase, clinicId, actualTokens);

      // F-AI-08: Audit log the AI invocation (no PHI — only counts/tokens)
      void logAuditEvent({
        supabase,
        action: "ai.patient_summary_generated",
        type: "patient",
        clinicId,
        actor: doctorId,
        ...
        metadata: {
          ...
          forced_refresh: data.forceRefresh,
        },
      });
      ...
      // A72-F2: Watermark the AI-generated summary (EU AI Act Art.50)
      // The cached version stores unwatermarked text to avoid double-marking.
      const watermarkedSummary = watermarkAiContent(summary, "patient-summary");

      // A80-F1: Per-tenant cost metering (non-fatal, fire-and-forget)
      void incrementClinicMetric(undefined, clinicId, "ai_patient_summaries");
      void incrementClinicMetric(undefined, clinicId, "openai_tokens", actualTokens);

      // F-AI-04 / A199: Include mandatory clinical disclaimer in all AI summary responses.
      return apiSuccess<PatientSummaryResponse>({
        summary: watermarkedSummary,
        generatedAt,
        patientId: data.patientId,
        cached: false,
