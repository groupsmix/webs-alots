...
export async function GET(request: NextRequest) {
  ...
  // A46-08: Use safe integer parsing — Number("abc") === NaN, which causes
  // Supabase to send malformed queries. Default to safe values on failure.
  ...
  if (search) {
    // A46-03: Sanitize search input to prevent PostgREST filter injection.
    // Strip %, _, comma, parens, dots AND pipe | (PostgREST OR separator).
    // The previous implementation missed | which allowed `.or()` injection.
