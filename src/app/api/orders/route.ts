...
const createOrderSchema = z.object({
  ...
  order_source: z
    .enum(["in_person", "qr_code", "whatsapp", "phone"])
    .optional()
    .default("in_person"),
});
...
export const GET = withAuth(
  async (request: NextRequest, auth: AuthContext) => {
    ...
    let query = auth.supabase
      .from("orders")
      .select(
        "id, clinic_id, reservation_id, table_id, items, subtotal, tax_amount, total, status, order_source, created_at, updated_at",
      )
...
export const POST = withAuthValidation(
  ...
  async (body, _request, auth) => {
    ...
    const taxAmount = Math.round(subtotal * 0.2 * 100) / 100; // 20% TVA standard Morocco
