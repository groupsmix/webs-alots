...
export const GET = withAuth(async () => {
  try {
    ...
    // 1. WhatsApp
    const hasWhatsApp =
      !missing.some((m) => m.name === "WHATSAPP_PHONE_NUMBER_ID") ||
      !missing.some((m) => m.name === "TWILIO_ACCOUNT_SID");
    services.push({
      name: "WhatsApp API",
    ...
    // 2. R2 Storage
    const hasR2 =
      !missing.some((m) => m.name === "R2_ACCOUNT_ID") &&
      !missing.some((m) => m.name === "R2_ACCESS_KEY_ID");
    services.push({
      name: "Storage (R2)",
    ...
    // 3. Payments
    const hasPayments =
      !warnings.some((w) => w.name === "STRIPE_SECRET_KEY") ||
      !warnings.some((w) => w.name === "CMI_MERCHANT_ID");
    services.push({
