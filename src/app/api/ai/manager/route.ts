...
function buildUserMessage(
  ...
  if (metrics.inactivePatients.length > 0) {
    // F-AI-02: Do NOT include patient names (PHI) in the AI prompt.
    // The count is sufficient for the manager's analytics use-case.
    parts.push(`- Exemples patients inactifs: ${metrics.inactivePatientsCount} patients identifiés`);
  }
...
export const POST = withAuthValidation(
  ...
  async (data, _request: NextRequest, auth) => {
    ...
    const estimatedTokens = estimateTokens(data.question) + 1000; // Add buffer for system prompt + metrics
    ...
    try {
      ...
      // F-AI-08: Audit log the AI manager invocation
      void logAuditEvent({
        supabase,
        action: "ai.manager_query",
        type: "admin",
        clinicId,
        actor: userId,
        description: "AI manager dashboard query",
        metadata: {
          tokens_used: actualTokens,
          history_length: data.conversationHistory.length,
        },
      });
