...
async function fetchClinicContext(
  ...
  try {
    ...
    // F-AI-02: Sanitize DB-sourced strings (services.name, users.name) before
    // they are interpolated into the AI system prompt. These values are
    // operator-configured and could contain prompt-injection payloads.
    return {
      name: sanitizeUntrustedText(clinic.name ?? "Clinic"),
      phone: clinic.phone ?? "",
      address: clinic.address ? sanitizeUntrustedText(clinic.address) : null,
      services: (servicesRes.data ?? [])
        .map((s) => sanitizeUntrustedText(s.name ?? ""))
        .filter(Boolean),
      doctors: (doctorsRes.data ?? [])
        .map((d) => sanitizeUntrustedText(d.name ?? ""))
        .filter(Boolean),
      openingHours: cfg?.opening_hours ? sanitizeUntrustedText(String(cfg.opening_hours)) : null,
    };
  } catch (err) {
...
async function generateAIResponse(
  message: string,
  ctx: ClinicContext,
  clinicId: string, // Add clinicId parameter for budget tracking
): Promise<string> {
  ...
  const supabase = createAdminClient();
  const estimatedTokens = estimateTokens(message) + 300; // Add buffer for system prompt

  // For WhatsApp receptionist, use a default "receptionist" role for budget limits
  ...
  if (!allowed) {
    return `Merci pour votre message. Notre quota IA mensuel est atteint. Appelez-nous au ${ctx.phone} pour assistance immédiate.`;
  }
...
async function handleIncomingMessage(
  ...
  const aiReply = await generateAIResponse(text, ctx, clinicId);
