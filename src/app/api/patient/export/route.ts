...
export const GET = withAuth(async (request: NextRequest, { supabase, profile }) => {
  ...
  const [
    ...
    { data: payments },
    { data: documents },
    // A62-F4: Include consent_logs, notification_log, and activity_logs
    // Law 09-08 Art.15/20 requires ALL personal data to be exportable.
    { data: consentLogs },
    ...
    supabase
      ...
      .order("created_at", { ascending: false })
      .limit(EXPORT_ROW_LIMIT),
    // Consent history (what the patient agreed to and when)
    supabase
      .from("consent_logs")
      .select("id, consent_type, granted, granted_at, revoked_at, version, created_at")
      .eq("patient_id", profile.id)
      .order("created_at", { ascending: false })
      .limit(EXPORT_ROW_LIMIT),
    // WhatsApp/SMS/email notification history involving this patient
    supabase
      .from("notification_log")
      .select("id, type, status, sent_at, created_at")
      // @ts-expect-error -- Supabase generated types lag behind actual DB schema
      .eq("patient_id", profile.id)
      .order("created_at", { ascending: false })
      .limit(EXPORT_ROW_LIMIT),
    // Activity log entries where this patient was the actor (their own actions)
    supabase
      .from("activity_logs")
      .select("id, action, type, description, timestamp")
      .eq("actor", profile.id)
  ...
  const exportData = {
    ...
    exportNote: "This export contains all personal data held about you under Moroccan Law 09-08 Art.15 (right of access) and Art.20 (right to portability).",
    personalInfo: {
    ...
    payments: payments ?? [],
    documents: documents ?? [],
    // A62-F4: Previously missing from export — now included
    consentHistory: consentLogs ?? [],
    notificationHistory: notificationLog ?? [],
    myActivityLog: activityLogs ?? [],
  };
