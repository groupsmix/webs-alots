...
async function handler(request: NextRequest, { supabase, profile }: AuthContext): Promise<NextResponse> {
  ...
  if (profile.role === "patient" && profile.clinic_id) {
    ...
    // M-02: Deny access if no patient_files record is found.
    // Previously, the absence of a record was treated as "legacy file" and
    // access was granted. This creates an IDOR: a patient who knows another
    // patient's R2 key prefix can download their files if those files predate
    // the patient_files table.
    //
    // To backfill ownership for legacy files, run:
    //   npm run backfill:patient-files
    //
    // After the backfill is complete and verified, remove the "legacy" comment
    // from the SOP and treat all unrecorded keys as unauthorized.
    if (!fileRecord) {
      logger.warn("Patient download denied: no patient_files record (run backfill:patient-files)", {
        context: "api/files/download",
        role: profile.role,
        profileId: profile.id,
        keyPrefix: baseKey.split("/").slice(0, 2).join("/"),
      });
      return apiForbidden("File ownership cannot be verified. Please contact clinic staff.");
    }
