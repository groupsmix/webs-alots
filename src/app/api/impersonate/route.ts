...
export const POST = withAuthValidation(impersonateSchema, async (body, request, { supabase, user }) => {
    const { clinicId, clinicName, password, reason, mfaCode } = body;

    // A35: MFA Step-Up - Verify TOTP code before granting impersonation
    // This ensures super-admin impersonation requires multi-factor authentication
    const { verified, error: mfaError } = await requireMfa(mfaCode, "impersonate");
    
    if (!verified) {
      logger.warn("MFA verification failed for impersonation attempt", {
      ...
      return apiUnauthorized(
        mfaError === "mfa.invalidCode" 
          ? "Invalid MFA code. Please check your authenticator app and try again."
          : "MFA verification failed. Please ensure you have MFA enabled."
      );
    }
    ...
    await logSecurityEvent({
      ...
      metadata: { reason, targetClinicId: clinicId, ipAddress: clientIp, userAgent, mfaVerified: true },
    });
