...
export const GET = withAuth(
  async (_request, auth) => {
    ...
    const cndpData =
      cndpRes.status === "fulfilled" ? (cndpRes.value.data as { status: string } | null) : null;

    return apiSuccess({
      cndpStatus: cndpData?.status ?? null,
      openDsars: openDsarsRes.status === "fulfilled" ? (openDsarsRes.value.count ?? 0) : 0,
      overdueDsars: overdueDsarsRes.status === "fulfilled" ? (overdueDsarsRes.value.count ?? 0) : 0,
      activeBreaches: breachesRes.status === "fulfilled" ? (breachesRes.value.count ?? 0) : 0,
    });
  },
  ["super_admin"],
);
