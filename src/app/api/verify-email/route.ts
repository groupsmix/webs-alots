...
import { timingSafeEqual, sha256Hex } from "@/lib/crypto-utils";
...
export const POST = withValidation(verifyEmailSendSchema, async (data, request: NextRequest) => {
  ...
  // A8-01: Never log email addresses - use hashed identifier for correlation
  logger.info("Email verification code generated", { 
    context: "verify-email", 
    emailHash: await sha256Hex(email) // Hash for correlation without PII
  });
