...
async function verifyBookingToken(token: string): Promise<BookingTokenResult> {
  ...
  // A6-13: Backward compatibility handling for old 3-part tokens
  if (parts.length === 3) {
    // Old format: phone:expiry:signature (no clinicId)
    logger.info("Old 3-part booking token detected", {
      context: "booking/token-verification",
      tokenFormat: "legacy-3-part",
    });
    return { valid: false }; // Reject old tokens with user-friendly message
  }
  
  // New format: clinicId:phone:expiry:signature (4 parts)
  if (parts.length !== 4) {
...
export const POST = withValidation(bookingRequestSchema, async (body, request: NextRequest) => {
    ...
    if (!tokenResult.valid) {
      // A6-13: Provide user-friendly error message for old tokens and general failures
      const parts = bookingToken.split(":");
      if (parts.length === 3) {
        logger.info("Old 3-part booking token rejected", {
          context: "booking",
          tokenFormat: "legacy-3-part",
        });
        return apiForbidden(
          "Your booking link has expired. Please request a new one."
        );
      } else {
        logger.warn("Invalid booking token rejected", {
          context: "booking",
          tokenFormat: parts.length === 4 ? "new-4-part-invalid" : "unknown",
        });
        return apiForbidden(
          "Your booking link has expired or is invalid. Please request a new verification code."
        );
      }
    }
    ...
    if (tokenResult.clinicId !== clinicId) {
      logger.warn("Cross-tenant booking token rejected", {
        context: "booking",
        tokenClinicId: tokenResult.clinicId,
    ...
    // TOCTOU race under concurrent requests (see A96-01 / F-A96-01 audit
    // finding). The booking_atomic_insert RPC wraps slot-count check + insert
    // inside a single transaction with a pg_advisory_xact_lock, guaranteeing
    // that maxPerSlot can never be exceeded.
