...
describe("Impersonate API — validation", () => {
  ...
  it("accepts valid impersonation payload with MFA code", async () => {
  ...
  it("rejects impersonation without MFA code", async () => {
    const { impersonateSchema } = await import("@/lib/validations");
    const result = impersonateSchema.safeParse({
      clinicId: "clinic-123",
      password: "admin-password",
  ...
  it("rejects MFA code shorter than 6 characters", async () => {
    const { impersonateSchema } = await import("@/lib/validations");
    const result = impersonateSchema.safeParse({
      clinicId: "clinic-123",
      password: "admin-password",
  ...
  it("rejects MFA code longer than 6 characters", async () => {
    const { impersonateSchema } = await import("@/lib/validations");
    const result = impersonateSchema.safeParse({
      clinicId: "clinic-123",
      password: "admin-password",
