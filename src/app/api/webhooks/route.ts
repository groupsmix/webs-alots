...
/**
 * A44-5: Replay protection — reject webhooks that carry a timestamp
 * older than WEBHOOK_MAX_AGE_MS (5 minutes). WhatsApp includes Unix
 * timestamps in status update objects. Replayed webhooks contain stale
 * timestamps and can be detected and dropped here.
 *
 * Note: WhatsApp doesn't include a top-level timestamp on message
 * notifications (only on status updates). For message events without
 * a timestamp, we cannot apply this check — signature verification
 * is the primary protection for those.
 */
const WEBHOOK_MAX_AGE_MS = 5 * 60 * 1000; // 5 minutes

function isWebhookReplay(body: WaWebhookBody): boolean {
  const entries = body.entry ?? [];
  for (const entry of entries) {
    for (const change of entry.changes ?? []) {
      for (const status of change.value?.statuses ?? []) {
        if (typeof status.timestamp === "string") {
          const eventMs = parseInt(status.timestamp, 10) * 1000;
          if (!Number.isNaN(eventMs) && Date.now() - eventMs > WEBHOOK_MAX_AGE_MS) {
            return true;
          }
        }
      }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // A44-5: Replay protection — drop webhooks with stale timestamps.
    // Signature verification prevents forgery; timestamp check prevents
    // legitimate-but-replayed webhooks from triggering duplicate mutations.
    if (isWebhookReplay(body)) {
      logger.warn("WhatsApp webhook replay detected — stale timestamp", {
        context: "webhooks",
      });
      // Return 200 so Meta doesn't retry — the webhook is genuine but stale.
      return apiSuccess({ status: "replay_ignored" });
    }
