...
import { Brain, Plus, Shield, Lock, Pill, Save, AlertTriangle, Eye, EyeOff } from "lucide-react";
...
import {
  fetchPsychSessionNotes,
  createPsychSessionNote,
  fetchPsychMedications,
  createPsychMedication,
  updatePsychMedication,
...
export default function PsychiatryPage() {
  ...
  const handleAddSession = async () => {
    ...
    const newId = await createPsychSessionNote({
      ...
      is_confidential: sessionForm.isConfidential,
      ...
      setSessions((prev) => [
        {
          ...
          observations: sessionForm.observations,
          plan: sessionForm.plan,
          isConfidential: sessionForm.isConfidential,
  ...
  const handleAddMed = async () => {
    ...
    const newId = await createPsychMedication({
      ...
      patient_id: user.id,
      doctor_id: user.id,
      medication_name: medForm.medicationName,
  ...
  const handleMedStatus = async (id: string, status: string) => {
    ...
    if (ok)
      setMedications((prev) =>
        prev.map((m) => (m.id === id ? { ...m, status, endDate: endDate ?? m.endDate } : m)),
      );
  };
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {s.moodRating && <Badge variant="outline">Mood: {s.moodRating}/10</Badge>}
                        </div>
                      </div>
                ...
                <Card key={med.id}>
                  <CardContent className="p-4">
                    ...
                    <div className="grid grid-cols-3 gap-3 text-sm mb-2">
                      ...
                      <div>
                        <span className="text-muted-foreground">Started:</span> {med.startDate}
                      </div>
                    </div>
                    {med.reason && (
                      <p className="text-sm text-muted-foreground mb-1">Reason: {med.reason}</p>
                    )}
                    {med.sideEffects && (
                    ...
                    {med.status === "active" && (
                      <div className="flex gap-2 mt-3">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleMedStatus(med.id, "discontinued")}
                        >
                          Discontinue
                        </Button>
                        <Button
                          ...
                          onClick={() => handleMedStatus(med.id, "completed")}
                        >
                          Complete Course
                        </Button>
                      </div>
            ...
            <CardContent>
              <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3 p-3 rounded-lg border">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Only the treating psychiatrist can view notes
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 p-3 rounded-lg border">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Visible to authorized members of the care team
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 p-3 rounded-lg border">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Visible to all clinic staff with patient access
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="text-sm text-muted-foreground">
                  <p>
                    Total confidential records: {sessions.filter((s) => s.isConfidential).length}
                  </p>
                  <p>
                    Doctor-only records:{" "}
                    {sessions.filter((s) => s.accessLevel === "doctor_only").length}
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setSessionForm((p) => ({ ...p, sessionType: v }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    id="confidential"
                    checked={sessionForm.isConfidential}
                    onChange={(e) =>
                      setSessionForm((p) => ({ ...p, isConfidential: e.target.checked }))
                    }
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={sessionForm.accessLevel}
                    onValueChange={(v) => setSessionForm((p) => ({ ...p, accessLevel: v }))}
                  >
      ...
      <Dialog open={showMedForm} onOpenChange={setShowMedForm}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 mt-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setMedForm((p) => ({ ...p, frequency: v }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="flex gap-2 justify-end">
              <Button variant="outline" onClick={() => setShowMedForm(false)}>
                Cancel
              </Button>
