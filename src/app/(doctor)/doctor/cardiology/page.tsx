...
import {
  ...
  fetchHeartMonitoringNotes,
  createHeartMonitoringNote,
  type ECGRecordView,
...
function bpCategory(systolic: number, diastolic: number): { label: string; color: string } {
  ...
  if (systolic >= 180 || diastolic >= 120)
    return { label: "Hypertensive Crisis", color: "text-red-800" };
...
export default function CardiologyPage() {
  ...
  const handleAddEcg = async () => {
    ...
    const newId = await createECGRecord({
      ...
      notes: ecgForm.notes,
      is_abnormal: ecgForm.isAbnormal,
    });
    if (newId) {
      setEcgs((prev) => [
        {
    ...
    <div>
      ...
      <Tabs defaultValue="ecg">
                ...
                <Card key={ecg.id}>
                  <CardContent className="p-4">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        <span className="text-muted-foreground">Rhythm:</span> {ecg.rhythm || "—"}
                      </div>
                      <div>
        ...
        <TabsContent value="bp">
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              <Card>
                <CardContent className="p-4 text-center">
                  ...
                  <p
                    className={`text-xs font-medium ${bpCategory(bpReadings[0].systolic, bpReadings[0].diastolic).color}`}
                  >
              ...
              <Card>
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-2xl font-bold">
                    {Math.round(
                      bpReadings.reduce((sum, r) => sum + r.systolic, 0) / bpReadings.length,
                    )}
                  </p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-2xl font-bold">
                    {Math.round(
                      bpReadings.reduce((sum, r) => sum + r.diastolic, 0) / bpReadings.length,
                    )}
                  </p>
                </CardContent>
              </Card>
          ...
          {bpReadings.length > 0 && (
            <Card className="mb-4">
              <CardHeader>
                <CardTitle className="text-sm">Blood Pressure History</CardTitle>
              </CardHeader>
              <CardContent>
      ...
      <Dialog open={showEcgForm} onOpenChange={setShowEcgForm}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Add ECG Record</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setEcgForm((p) => ({ ...p, rhythm: v }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="flex gap-2 justify-end">
              <Button variant="outline" onClick={() => setShowEcgForm(false)}>
                Cancel
              </Button>
              <Button onClick={handleAddEcg}>
          ...
          <div className="space-y-4 mt-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Select
                  value={bpForm.position}
                  onValueChange={(v) => setBpForm((p) => ({ ...p, position: v }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={bpForm.arm}
                  onValueChange={(v) => setBpForm((p) => ({ ...p, arm: v }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="flex gap-2 justify-end">
              <Button variant="outline" onClick={() => setShowBpForm(false)}>
                Cancel
              </Button>
              <Button onClick={handleAddBp}>
                <Save className="h-4 w-4 mr-1" /> Save
              </Button>
            </div>
          </div>
      ...
      <Dialog open={showNoteForm} onOpenChange={setShowNoteForm}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Add Monitoring Note</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-4">
            <div className="space-y-2">
              <Label>Title</Label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setNoteForm((p) => ({ ...p, category: v }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setNoteForm((p) => ({ ...p, severity: v }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="flex items-center gap-2">
              <input
                type="checkbox"
                id="isAlert"
                checked={noteForm.isAlert}
                onChange={(e) => setNoteForm((p) => ({ ...p, isAlert: e.target.checked }))}
              />
              <Label htmlFor="isAlert" className="text-sm">
                Flag as alert
              </Label>
            </div>
            <div className="flex gap-2 justify-end">
