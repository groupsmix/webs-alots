...
import { Plus, Save, Activity, Target } from "lucide-react";
...
import {
  fetchJointAssessments,
  createJointAssessment,
  fetchMobilityTests,
  createMobilityTest,
...
export default function RheumatologyPage() {
  ...
  const [mobilityForm, setMobilityForm] = useState({
    testType: "rom",
    joint: "",
    rangeOfMotion: {} as Record<string, string>,
    strengthScore: "",
    painDuringTest: "",
    notes: "",
  });
  ...
  const handleAddAssessment = async () => {
    ...
    const newId = await createJointAssessment({
      ...
      vas_pain_score: assessmentForm.vasPainScore
        ? parseInt(assessmentForm.vasPainScore)
        : undefined,
      morning_stiffness_minutes: assessmentForm.morningStiffnessMinutes
        ? parseInt(assessmentForm.morningStiffnessMinutes)
        : undefined,
      swollen_joint_count: swollenCount,
      tender_joint_count: tenderCount,
      functional_status: assessmentForm.functionalStatus,
      ...
      setAssessments((prev) => [
        {
          ...
          das28Score: null,
          functionalStatus: assessmentForm.functionalStatus,
          notes: assessmentForm.notes,
        },
        ...prev,
      ]);
  ...
  const handleAddMobility = async () => {
    ...
    const newId = await createMobilityTest({
      ...
      patient_id: user.id,
      doctor_id: user.id,
      test_type: mobilityForm.testType,
      joint: mobilityForm.joint,
      range_of_motion: rom,
      strength_score: mobilityForm.strengthScore ? parseInt(mobilityForm.strengthScore) : undefined,
      pain_during_test: mobilityForm.painDuringTest
        ? parseInt(mobilityForm.painDuringTest)
        : undefined,
      ...
      setMobilityTests((prev) => [
        {
          ...
          painDuringTest: mobilityForm.painDuringTest
            ? parseInt(mobilityForm.painDuringTest)
            : null,
          notes: mobilityForm.notes,
        },
    ...
    <div>
                      ...
                      <div className="rounded-lg border p-3 mb-3">
                        ...
                        <div className="flex gap-4 mt-2 text-[10px] text-muted-foreground">
                          <span className="flex items-center gap-1">
                            <span className="w-2 h-2 rounded-full bg-red-500" /> Both
                          </span>
                          <span className="flex items-center gap-1">
                            <span className="w-2 h-2 rounded-full bg-orange-500" /> Swollen
                          </span>
                          <span className="flex items-center gap-1">
                            <span className="w-2 h-2 rounded-full bg-yellow-500" /> Tender
                          </span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-sm">
                          {MOBILITY_TEST_TYPES.find((mt) => mt.value === t.testType)?.label ??
                            t.testType}
                        </span>
                    ...
                    <div className="grid grid-cols-3 gap-3 text-sm mb-2">
                      ...
                      {t.painDuringTest !== null && (
                        <div>
                          ...
                          <p className={`font-medium ${vasColor(t.painDuringTest)}`}>
                            {t.painDuringTest}/10
                          </p>
                        </div>
                      )}
      ...
      <Dialog open={showAssessmentForm} onOpenChange={setShowAssessmentForm}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>New Joint Assessment</DialogTitle>
          </DialogHeader>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="0"
                  max="10"
                  value={assessmentForm.vasPainScore}
                  onChange={(e) =>
                    setAssessmentForm((p) => ({ ...p, vasPainScore: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  type="number"
                  placeholder="30"
                  value={assessmentForm.morningStiffnessMinutes}
                  onChange={(e) =>
                    setAssessmentForm((p) => ({ ...p, morningStiffnessMinutes: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-2">
                ...
                <Select
                  value={assessmentForm.functionalStatus}
                  onValueChange={(v) => setAssessmentForm((p) => ({ ...p, functionalStatus: v }))}
                >
                  <SelectTrigger>
      ...
      <Dialog open={showMobilityForm} onOpenChange={setShowMobilityForm}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Add Mobility Test</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Select
                  value={mobilityForm.testType}
                  onValueChange={(v) => setMobilityForm((p) => ({ ...p, testType: v }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {MOBILITY_TEST_TYPES.map((t) => (
              ...
              <div className="space-y-2">
                <Label>Joint</Label>
                <Select
                  value={mobilityForm.joint}
                  onValueChange={(v) => setMobilityForm((p) => ({ ...p, joint: v }))}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select" />
                  </SelectTrigger>
                  ...
                  <div key={m} className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setMobilityForm((p) => ({
                          ...p,
                          rangeOfMotion: { ...p.rangeOfMotion, [m]: e.target.value },
                        }))
                      }
                    />
                  </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="0"
                  max="5"
                  value={mobilityForm.strengthScore}
                  onChange={(e) =>
                    setMobilityForm((p) => ({ ...p, strengthScore: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="0"
                  max="10"
                  value={mobilityForm.painDuringTest}
                  onChange={(e) =>
                    setMobilityForm((p) => ({ ...p, painDuringTest: e.target.value }))
                  }
                />
              </div>
            </div>
            ...
            <div className="flex gap-2 justify-end">
              <Button variant="outline" onClick={() => setShowMobilityForm(false)}>
                Cancel
              </Button>
              <Button onClick={handleAddMobility}>
                <Save className="h-4 w-4 mr-1" /> Save
              </Button>
            </div>
          </div>
