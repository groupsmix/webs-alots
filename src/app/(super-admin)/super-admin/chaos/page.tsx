...
export default function ChaosEngineeringPage() { 
  ...
  async function toggleChaos() { 
    ...
    try { 
      ...
      addToast( 
        chaosEnabled ? "Chaos experiments stopped" : "Chaos experiments started", 
        "success" 
      ); 
    } catch (err) { 
          ...
          return ( 
            <Card key={category}> 
              <CardHeader> 
                ...
                <CardDescription> 
                  {experiments.length} experiments configured 
                </CardDescription> 
              </CardHeader> 
              <CardContent className="space-y-3"> 
                {experiments.map(exp => ( 
                  <div key={exp.id} className="flex items-start justify-between py-2 border-b last:border-0"> 
      ...
      <Card className="mt-6"> 
        ...
        <CardContent className="prose prose-sm max-w-none"> 
          <ol> 
            ...
            <li>Run smoke tests to trigger failures: <code>npm run test:e2e</code></li> 
