...
import type { SuperAdminFeatureFlag } from "@/lib/super-admin-feature-flags";

const CATEGORY_LABELS: Record<SuperAdminFeatureFlag["category"], string> = {
  core: "Fonctionnalités principales",
  experimental: "Fonctionnalités expérimentales",
  integration: "Intégrations",
};

type FeatureFlagCategory = SuperAdminFeatureFlag["category"];

interface FeatureFlagsClientProps {
  initialFlags: SuperAdminFeatureFlag[];
  kvAvailable: boolean;
}

export default function FeatureFlagsClient({ initialFlags, kvAvailable }: FeatureFlagsClientProps) {
  ...
  const [flags, setFlags] = useState<SuperAdminFeatureFlag[]>(initialFlags);
  const [kvReady, setKvReady] = useState(kvAvailable);

  async function loadFlags(showRefreshing = false) {
    if (showRefreshing) {
      setRefreshing(true);
    } else {
      setLoading(true);
    ...
    try {
      const response = await fetch("/api/super-admin/feature-flags", { cache: "no-store" });
      const payload = (await response.json()) as {
        ok: boolean;
        data?: { flags: SuperAdminFeatureFlag[]; kvAvailable: boolean };
        error?: string;
      };
      ...
      setKvReady(payload.data.kvAvailable);
    } catch (error) {
      logger.warn("Failed to load feature flags", { context: "super-admin/feature-flags", error });
  ...
  const groupedFlags = useMemo(() => {
    return flags.reduce<Record<FeatureFlagCategory, SuperAdminFeatureFlag[]>>(
      (acc, flag) => {
  ...
  return (
    <div>
      ...
      {!loading &&
        (Object.entries(groupedFlags) as Array<[FeatureFlagCategory, SuperAdminFeatureFlag[]]>)
                ...
                {categoryFlags.map((flag) => (
                  ...
                  >
                    ...
                    <Switch
                      ...
                      disabled={updating === flag.key || flag.locked || !kvReady}
                    />
                  </div>
                ))}
              </CardContent>
