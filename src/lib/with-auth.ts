...
// A12-02: Use LRU cache with TTL to prevent unbounded memory growth and DoS attacks.
...
/**
 * A12-02: Replace Map with LRU cache to prevent resource leak DoS.
 * - max: 10000 entries (prevents unbounded growth)
 * - ttl: 60000ms (1 minute, matches rate window)
...
const userRateBuckets = new LRUCache<string, UserRateEntry>({
  ...
  // Log evictions for monitoring DoS attempts
  dispose: (value, key) => {
...
export function withAuth(
  ...
  return async (request: NextRequest): Promise<NextResponse> => {
    try {
      ...
      // F-A93-04: Log 100% of authenticated read access.
      // Moroccan Law 09-08 requires ALL authenticated access to be auditable.
      // Sampling = deletion. Manage volume via log-retention.md, not here.
