 ...
 */
export async function resolveAIConfig(clinicId?: string): Promise<
...
> {
  ...
  if (clinicId) {
    ...
    if (!budget.allowed) {
      logger.warn("AI budget exceeded", { context: "ai-config", clinicId, usage: budget.usage, limit: budget.limit });
      return { ok: false, reason: "Monthly AI usage limit exceeded (110%). Please upgrade your plan.", statusCode: 429 };
    }
  }
 ...
 */
export async function checkClinicAIBudget(clinicId: string): Promise<{ allowed: boolean; usage: number; limit: number }> {
  try {
    const supabase = createUntypedAdminClient("ai-config-resolve");
    ...
    if (clinicData?.config && typeof clinicData.config === 'object' && 'ai_monthly_budget' in clinicData.config) {
    ...
    logger.warn("Failed to check AI budget, failing open", { context: "ai-config", clinicId, error: err });
