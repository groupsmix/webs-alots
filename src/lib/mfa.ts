...
/**
 * Require MFA verification for sensitive operations (e.g., impersonation).
 * 
 * This function verifies a TOTP code for users who have MFA enabled.
 * It creates a challenge and verifies the provided code against the user's
 * first verified TOTP factor.
 * 
 * @param mfaCode - The TOTP code provided by the user
...
export async function requireMfa(
  ...
  if (!user) {
    return { verified: false, error: "auth.genericError" };
  }

  // Check if user has MFA enabled
  const { factors, error: listError } = await getMFAFactors();
  
  if (listError) {
    logger.warn("Failed to list MFA factors during step-up", {
      context: "mfa/requireMfa",
      operation,
      error: listError,
    });
    return { verified: false, error: "mfa.verifyError" };
  }

  const verifiedFactors = factors.filter((f) => f.status === "verified");
  
  if (verifiedFactors.length === 0) {
    // User doesn't have MFA enabled - this should be enforced at a higher level
    logger.warn("MFA step-up attempted for user without MFA", {
      context: "mfa/requireMfa",
      operation,
      userId: user.id,
    });
    return { verified: false, error: "mfa.notEnrolled" };
  }

  // Use the first verified factor
  const factorId = verifiedFactors[0].id;

  // Log MFA challenge for audit
  await logAuthEvent({
    supabase,
    action: "mfa.challenge_issued",
    actor: user.email ?? user.id,
    description: `MFA challenge issued for ${operation}`,
    success: true,
    metadata: { operation },
  }).catch((err) => {
    logger.warn("Failed to log MFA challenge", {
      context: "mfa/requireMfa",
      error: err,
    });
  });
  ...
  if (challengeError) {
    logger.warn("MFA challenge failed during step-up", {
      context: "mfa/requireMfa",
      operation,
      error: challengeError,
    });
    ...
    await logAuthEvent({
      ...
      actor: user.email ?? user.id,
      description: `MFA challenge failed for ${operation}`,
      success: false,
      metadata: { operation, error: challengeError.message },
    }).catch(() => {});
  ...
  if (verifyError) {
    logger.warn("MFA verification failed during step-up", {
      context: "mfa/requireMfa",
      operation,
      error: verifyError,
    });
    ...
    await logAuthEvent({
      ...
      actor: user.email ?? user.id,
      description: `MFA verification failed for ${operation}`,
      success: false,
      metadata: { operation, error: verifyError.message },
    }).catch(() => {});
    
    return { verified: false, error: "mfa.invalidCode" };
  }
  ...
  await logAuthEvent({
    ...
    actor: user.email ?? user.id,
    description: `MFA verification successful for ${operation}`,
    success: true,
    metadata: { operation },
  }).catch((err) => {
    logger.warn("Failed to log MFA verification success", {
      context: "mfa/requireMfa",
      error: err,
    });
  });
