 ...
 */
/**
 * A36.5: Cloudflare IP ranges that are trusted to set CF-Connecting-IP.
 * These ranges are published at https://www.cloudflare.com/ips/
 * and should be updated periodically.
 * 
 * When CF-Connecting-IP is present, we trust it unconditionally because
 * Cloudflare Workers only run behind Cloudflare's edge. The TRUSTED_PROXIES
 * constant is documented here for completeness and future non-Cloudflare
 * deployments, but is not actively validated in the current implementation.
 * 
 * For non-Cloudflare deployments, the application MUST validate that the
 * request originates from a trusted proxy IP before trusting XFF/X-Real-IP.
 */
...
/**
 * A42.4: Per-user rate limiting at the edge.
 * Limits authenticated users to 100 requests per 60 seconds per user ID.
 * This prevents a single compromised account from overwhelming the system.
 * Applied in middleware after user authentication.
 */
...
/**
 * A42.4: Per-API-key rate limiting at the edge.
 * Limits API key usage to 1000 requests per 60 seconds per API key.
 * This prevents API key abuse while allowing legitimate high-volume integrations.
 * Applied in middleware for requests with Bearer tokens.
 */
export const perApiKeyLimiter = createRateLimiter({
  windowMs: 60_000,
  max: 1000,
  failClosed: true,
});
...
export const rateLimitRules: RateLimitRule[] = [
  ...
  { prefix: "/api/webhooks/cmi", limiter: webhookLimiter, windowMs: 60_000, max: 100 },
  ...
  // A36.4: Global catch-all rate limit fallback ensures no endpoint is unprotected.
  // This rule is checked last and applies to all paths not matched by more specific rules.
  // 100 requests per 60 seconds per IP — prevents abuse while allowing normal usage.
  { prefix: "/*", limiter: globalPageLimiter, windowMs: 60_000, max: 100 },
];
