...
async function deleteConsultationNote(id: string): Promise<boolean> {
  const supabase = createClient();
...
async function deleteOdontogramEntry(
  clinicId: string,
  patientId: string,
  toothNumber: number,
): Promise<boolean> {
...
async function updateSterilizationEntry(
  id: string,
  data: {
    tool_name?: string;
    method?: string;
...
async function updateBeforeAfterPhoto(
  id: string,
  data: {
    description?: string;
    category?: string;
...
async function deleteBeforeAfterPhoto(id: string): Promise<boolean> {
  const supabase = createClient();
