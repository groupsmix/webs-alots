...
/**
 * A1-01: Bound AI prescription inputs to prevent token exhaustion.
 * Diagnosis and symptoms are capped at 2000 chars each to ensure
 * predictable token consumption and prevent budget drain attacks.
 */
...
// ── Prescription Content (A16-06) ───────────────────────────────────────

/**
 * A16-06: JSONB schema validation for prescription content.
 * Validates the structure of prescription data before insertion into the
 * prescriptions.content JSONB column to prevent malformed data.
 */
...
/**
 * A1-01 / A14-01: bound `content` to 4 000 chars to prevent unbounded payloads
...
/**
 * A1-01: Cap messages array at 20 to prevent token exhaustion attacks.
 * Combined with per-message content limits, this ensures total request
 * size is bounded at ~80k chars (20 messages × 4k chars).
 */
...
/**
 * A1-01: Bound AI manager inputs to prevent token exhaustion.
 * Question and conversation history are capped to ensure predictable
 * token consumption. History is limited to 20 messages × 2000 chars = 40k chars max.
 */
...
/**
 * A1-01: Bound AI auto-suggest inputs to prevent token exhaustion.
 * Diagnosis is capped at 2000 chars to ensure predictable token consumption.
 */
...
// ── A62-F1: Patient profile self-update ──────────────────────────────────
// Used by POST /api/patient/profile to validate and audit patient profile edits.
// Only fields the patient controls are accepted; clinic_id/role are immutable.

export const patientProfileUpdateSchema = z.object({
  /** Display name — required if provided */
  name: z.string().min(1).max(200).optional(),
  /** Moroccan phone format: +212 prefix or local 06/07 */
  phone: z
    .string()
    .max(30)
  ...
  date_of_birth: z
    ...
    .optional(),
  /** Free-form address — max 500 chars */
  address: z.string().max(500).optional(),
  /** Optional blood type for clinical records */
  blood_type: z
    .enum(["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"])
    .optional(),
}).strict(); // Reject any unknown fields — immutable fields must not be accepted
