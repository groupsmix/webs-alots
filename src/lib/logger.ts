...
// A41: Enhanced with hostname, r2Key per Phase 2 Infrastructure Hardening
const PHI_FIELD_PATTERNS = new Set([
  ...
  // A8-01: Additional PII patterns identified in audit
  "full_name", "doctor_name", "clinic_name", "owner_name",
  "emergency_contact", "next_of_kin", "patient_address",
  "medical_history", "allergies", "medications", "symptoms",
  "insurance_id", "national_id", "passport", "license_number",
  "credit_card", "bank_account", "iban", "swift",
  // A41: Additional PII patterns from Phase 2 Infrastructure Hardening
  "hostname", "r2key", "r2_key",
]);

/**
 * Hash an R2 key for logging purposes (synchronous version).
 * A41: R2 keys may contain patient identifiers, so we hash them
 * and only log the first 8 characters for debugging correlation.
 * 
 * Note: Uses a simple hash for synchronous operation. For production,
 * consider using crypto.subtle.digest in an async context if available.
 */
function hashR2KeySync(key: string): string {
  // Simple hash implementation for synchronous context
  let hash = 0;
...
function redactPhi(obj: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (PHI_FIELD_PATTERNS.has(lowerKey)) {
      // A41: Special handling for R2 keys - hash instead of full redaction
      if ((lowerKey === "r2key" || lowerKey === "r2_key") && typeof value === "string") {
        result[key] = "[REDACTED]";
        result[`${key}Hash`] = hashR2KeySync(value);
      } else {
        result[key] = "[REDACTED]";
      }
