...
const STATUS_ORDER: IVFCycleStatus[] = [
  "planned",
  "stimulation",
  "retrieval",
  "fertilization",
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && (
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">New IVF Cycle</CardTitle>
          </CardHeader>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setForm({ ...form, cycleType: e.target.value as IVFCycleType })}
                    ...
                    <div>
                      ...
                      {cycle.partnerName && (
                        <p className="text-xs text-muted-foreground">
                          Partner: {cycle.partnerName}
                        </p>
                      )}
                      {cycle.doctorName && (
                        <p className="text-xs text-muted-foreground">Dr. {cycle.doctorName}</p>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                    ...
                    {STATUS_ORDER.filter((s) => s !== "cancelled").map((step, idx) => {
                      ...
                      return (
                        <div key={step} className="flex items-center">
                          <div
                            className={`px-2 py-0.5 rounded text-[10px] whitespace-nowrap ${isCurrent ? STATUS_COLORS[step] + " font-medium" : isPast ? "bg-green-50 text-green-600" : "bg-muted text-muted-foreground"}`}
                          >
                            {step.replace("_", " ")}
                          </div>
                          {idx < 6 && (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs text-muted-foreground">
                    {cycle.startDate && (
                      <div className="flex items-center gap-1">
                        <Calendar className="h-3 w-3" /> Start: {cycle.startDate}
                      </div>
                    )}
                    ...
                    {cycle.embryosTransferred !== null && (
                      <div>Transferred: {cycle.embryosTransferred}</div>
                    )}
                  ...
                  {editable && nextStatus && (
                    <div className="mt-3 flex gap-2 justify-end">
                      <Button
                        ...
                        variant="outline"
                        className="text-xs"
                        onClick={() => onAdvanceStatus?.(cycle.id, nextStatus)}
                      >
                      ...
                      {cycle.status === "tww" && (
                        <>
                          <Button
                            ...
                            variant="default"
                            className="text-xs"
                            onClick={() => onUpdateOutcome?.(cycle.id, "positive")}
                          >
                          ...
                          <Button
                            ...
                            variant="outline"
                            className="text-xs"
                            onClick={() => onUpdateOutcome?.(cycle.id, "negative")}
                          >
