...
import { predictWaitTime, type QueueState, type WaitTimeEstimate } from "@/lib/algorithms/wait-time-predictor";
...
export function EstimatedWaitTime({ className = "" }: { className?: string }) {
  ...
  const statusColor = 
    estimate.status === "on-time" ? "text-green-600 bg-green-50 border-green-200" :
    estimate.status === "delayed" ? "text-amber-600 bg-amber-50 border-amber-200" :
    "text-red-600 bg-red-50 border-red-200";

  const statusText = 
    estimate.status === "on-time" ? (lang === "ar" ? "في الموعد" : "À l'heure") :
    estimate.status === "delayed" ? (lang === "ar" ? "تأخير طفيف" : "Léger retard") :
    (lang === "ar" ? "تأخير كبير" : "Retard important");

  return (
    <Card className={`overflow-hidden ${className}`}>
      <div className={`h-1.5 w-full ${estimate.status === 'on-time' ? 'bg-green-500' : estimate.status === 'delayed' ? 'bg-amber-500' : 'bg-red-500'}`} />
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-col items-center justify-center py-2">
          ...
          <span className="text-sm text-muted-foreground mt-1">
            ({lang === "ar" ? "بين" : "Entre"} {estimate.timeRange.min} {lang === "ar" ? "و" : "et"} {estimate.timeRange.max} {lang === "ar" ? "دقيقة" : "min"})
          </span>
        </div>
