  ...
  const handleAdd = () => {
    if (form.patientName.trim() && form.sessionDate && onAdd) {
      onAdd({
        ...
        recurrencePattern:
          form.isRecurring && form.recurrencePattern
            ? (form.recurrencePattern as DialysisRecurrencePattern)
            : null,
      });
      setForm({
        patientName: "",
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && (
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">Schedule Dialysis Session</CardTitle>
          </CardHeader>
...
function SessionCard({
  ...
  session: SessionView;
  editable: boolean;
  onUpdateStatus?: (id: string, status: DialysisSessionStatus) => void;
}) {
        ...
        <div className="flex items-center gap-2">
          {session.isRecurring && session.recurrencePattern && (
            <Badge variant="outline" className="text-[10px]">
              ...
              {RECURRENCE_LABELS[session.recurrencePattern]}
            </Badge>
          )}
