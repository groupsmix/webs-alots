...
const STATUS_FLOW: ProstheticOrderStatus[] = [
  "received",
  "in_progress",
  "quality_check",
  "ready",
...
export function ProstheticOrders({
  orders,
  editable = false,
  onAdd,
  onAdvanceStatus,
}: ProstheticOrdersProps) {
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({
    dentistName: "",
    patientName: "",
    orderType: "crown" as ProstheticOrderType,
    material: "",
    shade: "",
    toothNumbers: "",
    dueDate: "",
    price: "",
    priority: "normal" as ProstheticPriority,
    specialInstructions: "",
  });
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && (
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">New Prosthetic Order</CardTitle>
          </CardHeader>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm({ ...form, orderType: e.target.value as ProstheticOrderType })
                  }
            ...
            <div className="grid grid-cols-4 gap-3">
              ...
              <div>
                ...
                <select
                  value={form.priority}
                  onChange={(e) =>
                    setForm({ ...form, priority: e.target.value as ProstheticPriority })
                  }
                  className="w-full rounded-md border px-3 py-2 text-sm"
                >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {order.toothNumbers.length > 0 && (
                          <Badge variant="outline" className="text-[10px]">
                            #{order.toothNumbers.join(", ")}
                          </Badge>
                        )}
                        <Badge variant={PRIORITY_VARIANTS[order.priority]} className="text-[10px]">
                          {order.priority}
                        </Badge>
                      </div>
                      <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                        ...
                        <span className="flex items-center gap-0.5">
                          <Clock className="h-3 w-3" /> {order.receivedDate}
                        </span>
                        {order.dueDate && (
                      ...
                      <div className="flex items-center gap-0.5">
                        {STATUS_FLOW.map((s, idx) => {
                          ...
                          return (
                            <div
                              key={s}
                              className={`h-1.5 w-4 rounded-full ${isPast ? "bg-primary" : "bg-muted"}`}
                              title={s}
                            />
                          );
                        })}
