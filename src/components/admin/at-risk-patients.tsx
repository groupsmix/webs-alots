...
import { CheckCircle2, MessageSquare, Phone, UserX } from "lucide-react";
...
import {
  ...
  type PatientHistoryFeatures,
} from "@/lib/algorithms/patient-churn-predictor";
  ...
  const getRiskBadge = (level: string) => {
    if (level === "high") {
      return (
        ...
        >
          {lang === "ar" ? "خطر مرتفع" : "Risque Élevé"}
        </Badge>
      );
    }
    return (
      <Badge variant="outline" className="bg-amber-100 text-amber-800 border-amber-200">
        {lang === "ar" ? "خطر متوسط" : "Risque Moyen"}
      </Badge>
    );
  };
        ...
        {patients.length === 0 ? (
          <div className="flex items-center gap-2 text-green-600 bg-green-50 p-4 rounded-md border border-green-100 justify-center">
            ...
            <span className="text-sm font-medium">
              {lang === "ar"
                ? "لا يوجد مرضى معرضون لخطر الانقطاع حالياً."
                : "Aucun patient à risque identifié actuellement."}
            </span>
          </div>
        ) : (
              ...
              >
                <div className="flex-1 space-y-2">
                  ...
                  <div className="text-xs text-muted-foreground grid grid-cols-2 gap-y-1">
                    <span>
                      ...
                      {Math.floor(patient.features.daysSinceLastVisit)}{" "}
                      {lang === "ar" ? "يوماً" : "jours"}
                    </span>
                    <span>
                      {lang === "ar" ? "إجمالي الزيارات:" : "Total visites :"}{" "}
                      {patient.features.totalVisits}
                    </span>
                    <span>
                      {lang === "ar" ? "معدل الإلغاء:" : "Taux d'annulation :"}{" "}
                      {Math.round(patient.features.cancellationRate * 100)}%
                    </span>
                  </div>
                ...
                <div className="flex flex-col gap-2 sm:w-48 bg-slate-50 dark:bg-slate-900/50 p-3 rounded-md justify-center">
                  ...
                  <p
                    ...
                    title={patient.prediction!.suggestedAction[lang]}
                  >
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-auto">
                    ...
                    <Button
                      size="sm"
                      className="w-full text-xs h-8 px-0 bg-[#25D366] hover:bg-[#128C7E]"
                      title="WhatsApp"
                    >
