...
import { AlertTriangle, ArrowDownRight, ArrowUpRight, CheckCircle2, Clock, Users } from "lucide-react";
import { useState, useEffect } from "react";
import { useLocale } from "@/components/locale-switcher";
import { Badge } from "@/components/ui/badge";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { optimizeStaffSchedule, type OptimizationResult, type HourlyVolume } from "@/lib/algorithms/staff-optimizer";
...
const generateMockData = (): HourlyVolume[] => {
  ...
  for (let day = 1; day <= 5; day++) {
    ...
    for (let hour = 8; hour <= 18; hour++) {
      let avgPatients = 0;
      let currentStaff = 2; // Default 2 staff
    ...
    <Card className={className}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className={`${result.criticalUnderstaffedHours > 0 ? 'bg-red-50 border-red-100' : 'bg-slate-50 border-slate-100'} p-3 rounded-lg flex flex-col justify-center`}>
            <span className={`text-xs font-medium mb-1 ${result.criticalUnderstaffedHours > 0 ? 'text-red-700' : 'text-slate-700'}`}>
              {lang === "ar" ? "ساعات النقص الحرج" : "Heures en sous-effectif critique"}
            </span>
            <span className={`text-xl font-bold flex items-center gap-1 ${result.criticalUnderstaffedHours > 0 ? 'text-red-700' : 'text-slate-700'}`}>
              ...
              {result.suggestions.slice(0, 5).map((sug, i) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-md border bg-card">
                  <div className={`mt-0.5 p-1.5 rounded-full ${
                    sug.status === 'understaffed' ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'
                  }`}>
                    {sug.status === 'understaffed' ? <ArrowUpRight className="h-4 w-4" /> : <ArrowDownRight className="h-4 w-4" />}
                  </div>
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant={sug.status === 'understaffed' ? 'destructive' : 'secondary'} className="text-[10px]">
