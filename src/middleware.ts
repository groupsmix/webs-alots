...
// ── A39: Network Segmentation Hardening ──────────────────────────

/**
 * A39.1: Egress filtering allowlist. Workers can only fetch() to these
 * external hosts. This prevents SSRF attacks where an attacker-controlled
 * URL parameter could be used to probe internal services or exfiltrate data.
 *
 * When adding a new external service, add its hostname here and document
 * the business justification in a code comment.
 */
...
/**
 * A39.3: DNS sanitization. Strip control characters from hostname parameters
 * to prevent DNS rebinding attacks and log injection. Control characters
 * (0x00-0x1F, 0x7F-0x9F) can be used to bypass allowlists or inject CRLF
 * sequences into logs.
 */
...
/**
 * A39.1: Validate that a fetch() URL is in the egress allowlist. Returns
 * true if the hostname is allowed, false otherwise. Handles both absolute
 * URLs and relative paths (relative paths are always allowed since they
 * target the same origin).
 */
function isEgressAllowed(url: string): boolean {
  try {
    // Relative URLs (e.g., "/api/...") are always allowed
    if (url.startsWith("/")) return true;

    const parsed = new URL(url);
    const sanitized = sanitizeHostname(parsed.hostname);

    // Check if the sanitized hostname is in the allowlist
...
// A2-08: Production feature flag validation (called once on first request)
let productionFlagsValidated = false;

export async function middleware(request: NextRequest) {
  // A2-08: Validate production feature flags on first request
  // This ensures the app fails fast if security-critical flags are misconfigured
  if (!productionFlagsValidated && process.env.NODE_ENV === "production") {
    try {
      ...
      // Production flag validation failed - return 503 to prevent app startup
  ...
  // A42.4: Per-user rate limiting at the edge (100 req/min per user)
  if (user?.id) {
    const { perUserLimiter } = await import("@/lib/rate-limit");
    const userAllowed = await perUserLimiter.check(`user:${user.id}`);
    if (!userAllowed) {
      const response = withSecurityHeaders(
        NextResponse.json(
          { error: "Too many requests from this user. Please try again later.", code: "USER_RATE_LIMIT_EXCEEDED" },
          { status: 429 },
        ),
        cspHeaders,
      );
  ...
  // A42.4: Per-API-key rate limiting at the edge (1000 req/min per API key)
  const authHeader = request.headers.get("authorization");
  if (authHeader?.startsWith("Bearer ")) {
    const apiKey = authHeader.slice(7); // Remove "Bearer " prefix
    // Only apply API key rate limiting if the key looks like a valid UUID or token
    // (not a Supabase JWT which starts with "eyJ")
    if (apiKey && !apiKey.startsWith("eyJ")) {
      const { perApiKeyLimiter } = await import("@/lib/rate-limit");
      const apiKeyAllowed = await perApiKeyLimiter.check(`apikey:${apiKey}`);
      if (!apiKeyAllowed) {
        const response = withSecurityHeaders(
          NextResponse.json(
            { error: "Too many requests from this API key. Please try again later.", code: "API_KEY_RATE_LIMIT_EXCEEDED" },
            { status: 429 },
          ),
          cspHeaders,
        );
