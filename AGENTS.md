...
1. **Never log secrets or PHI** — Use `@/lib/logger` for structured logging. Never `console.log` sensitive data.
   - **PII Redaction**: The logger automatically redacts PII fields (email, phone, name, etc.) via `redactPhi()`.
   - **Logging Pattern**: Only log UUIDs (clinicId, userId, patientId, appointmentId) — never log names, emails, or phone numbers.
   - **Audit Script**: Run `npm run audit:pii-logs` weekly to scan logs for PII leaks.
...
4. **Input validation** — All API inputs validated with Zod schemas defined in `@/lib/validations`.
   - **AI Input Limits**: Chat messages max 4000 chars, message arrays max 20 items, AI prompts max 2000 chars.
   - **AI Token Budget**: Enforced via `checkAiTokenBudget()` from `@/lib/ai-budget` before AI calls.
   - **Role-based Limits**: Patient: 10k tokens/month, Doctor: 50k, Receptionist: 30k, Clinic Admin: 100k.
...
7. **File uploads** — Magic byte validation + MIME type checking + path traversal prevention via `buildUploadKey()`.
   - **File Ownership Tracking**: All uploads tracked in `patient_files` table with clinic_id, patient_id, r2_key.
   - **Authorization**: Patients can only download own files; staff can download all clinic files.
   - **Backfill Script**: Run `npm run backfill:patient-files` to populate ownership for legacy files.
...
8. **Webhook signatures** — WhatsApp (HMAC-SHA256 via `X-Hub-Signature-256`) and Stripe (via `stripe-signature`) webhooks must be verified before processing.
   - **Timing-Safe Compare**: All signature verification uses `timingSafeEqual()` from `@/lib/crypto-utils`.
   - **DoS Protection**: Signatures capped at 1024 bytes to prevent CPU exhaustion attacks.

9. **Booking Token Tenant Binding** — Booking tokens include clinicId in format: `clinicId:phone:expiry:signature`.
   - **Cross-Tenant Protection**: Token verification checks clinicId matches expected clinic BEFORE signature verification.
   - **Breaking Change**: Old 3-part tokens return user-friendly error: "Your booking link has expired. Please request a new one."
